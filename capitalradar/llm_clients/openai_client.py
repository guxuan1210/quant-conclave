...
_PASSTHROUGH_KWARGS = (
    "timeout", "max_retries", "reasoning_effort", "request_timeout",
...
_PROVIDER_BASE_URL = {
    ...
    "ollama":     "http://172.20.86.180:11434/v1",
    "ollama2":    "http://172.20.86.254:11434/v1",
    "ollama3":    "http://172.20.86.180:11435/v1",
    "ollama4":    "http://172.20.86.254:11435/v1",
}

# ── Ollama multi-server round-robin ──
_ollama_rr_index = 0

def _ollama_next_url(config: dict = None) -> str:
    ...
    if not servers:
        ...
        servers = DEFAULT_CONFIG.get("ollama_servers", [_PROVIDER_BASE_URL["ollama"]])
    idx = _ollama_rr_index % len(servers)
    _ollama_rr_index += 1
    return servers[idx]
...
def _resolve_provider_base_url(provider: str) -> Optional[str]:
    ...
    Ollama (local): OLLAMA_BASE_URL env var override.
    Ollama2 (local2): OLLAMA2_BASE_URL env var override.
    """
    if provider == "ollama":
        ...
        if env_url:
            return env_url
    if provider == "ollama2":
        env_url = os.environ.get("OLLAMA2_BASE_URL")
        if env_url:
            return env_url
...
class OpenAIClient(BaseLLMClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Force-disable proxy via environment (overrides system proxy settings)
    ...
    def get_llm(self) -> Any:
        ...
        llm_kwargs = {"model": self.model, "timeout": 300, "request_timeout": 300}
        ...
        if self.provider in _PROVIDER_BASE_URL:
            if self.provider == "ollama" and not self.base_url:
                llm_kwargs["base_url"] = _ollama_next_url()
            else:
            ...
            # Disable SSL verification for LLM calls (corporate network SSL intercept)
            import httpx, ssl
            ctx = ssl.create_default_context()
            ctx.check_hostname = False
            ctx.verify_mode = ssl.CERT_NONE
            llm_kwargs["http_client"] = httpx.Client(proxy=None, verify=ctx, timeout=300)
        elif self.base_url:
        ...
        # Local reasoning models (e.g. qwen3.8:27b) default thinking ON on
        # Ollama, which makes every call tens of seconds slow. The capability
        # table marks them disable_thinking — inject chat_template_kwargs here
        # so ALL call sites get fast generation without each one passing it.
        # Caller-supplied extra_body (if any) is merged, never clobbered.
        if self.provider in ("ollama", "ollama2", "ollama3", "ollama4"):
            if get_capabilities(self.model).disable_thinking:
                _extra = llm_kwargs.setdefault("extra_body", {})
                _ctk = _extra.setdefault("chat_template_kwargs", {})
