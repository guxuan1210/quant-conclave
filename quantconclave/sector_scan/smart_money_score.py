...
from quantconclave.dataflows.quote_text import parse_quote_price, parse_quote_change_pct
...
def _dual_source_gate(ticker: str, flow_rows: List[Dict], config: dict) -> Dict:
    ...
    try:
        from quantconclave.sector_scan.moneyflow_verifier import verify_moneyflow
...
def _score_confirmation(rows: List[Dict], ticker: str, config: dict) -> Dict:
    ...
    # 2. Margin trend
    try:
        from quantconclave.agents.utils.capital_flow_tools import get_margin_trading
        end_date = datetime.now().strftime("%Y-%m-%d")
        start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d")
