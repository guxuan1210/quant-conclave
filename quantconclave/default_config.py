...
_QUANTCONCLAVE_HOME = os.path.join(os.path.expanduser("~"), ".quantconclave")
...
# their .env file.
_ENV_OVERRIDES = {
    "QUANTCONCLAVE_LLM_PROVIDER":         "llm_provider",
    "QUANTCONCLAVE_DEEP_THINK_PROVIDER":  "deep_think_provider",
    "QUANTCONCLAVE_QUICK_THINK_PROVIDER": "quick_think_provider",
    "QUANTCONCLAVE_DEEP_THINK_LLM":       "deep_think_llm",
    "QUANTCONCLAVE_QUICK_THINK_LLM":      "quick_think_llm",
    "QUANTCONCLAVE_LLM_BACKEND_URL":      "backend_url",
    "QUANTCONCLAVE_OUTPUT_LANGUAGE":      "output_language",
    "QUANTCONCLAVE_MAX_DEBATE_ROUNDS":    "max_debate_rounds",
    "QUANTCONCLAVE_MAX_RISK_ROUNDS":      "max_risk_discuss_rounds",
    "QUANTCONCLAVE_CHECKPOINT_ENABLED":   "checkpoint_enabled",
    "QUANTCONCLAVE_BENCHMARK_TICKER":     "benchmark_ticker",
    "QUANTCONCLAVE_PROXY":                "proxy",
    "TUSHARE_TOKEN":                      "tushare_token",
    "QUANTCONCLAVE_WECOM_WEBHOOK_URL":     "wecom_webhook_url",
    "QUANTCONCLAVE_WECOM_BOT_ID":          "wecom_bot_id",
    "QUANTCONCLAVE_WECOM_BOT_SECRET":      "wecom_bot_secret",
    "QUANTCONCLAVE_WECOM_BOT_ADVISOR_ENABLED": "wecom_bot_advisor_enabled",
    "QUANTCONCLAVE_WEB_ADVISOR_WECHAT_PUSH_ENABLED": "web_advisor_wechat_push_enabled",
    "QUANTCONCLAVE_ADVISORY_DATA_VERIFY": "advisory_data_verify",
...
_LEGACY_ENV_PREFIX = "CAPITALRADAR_"


def _legacy_env(name: str):
    """Read ``name``, falling back to its ``CAPITALRADAR_*`` predecessor.

    The pre-rename variables remain readable for one release so existing
    ``.env`` files and exports keep working; the new ``QUANTCONCLAVE_*``
    prefix always wins when both are set.
    """
    raw = os.environ.get(name)
    if raw is not None and raw != "":
        return raw
    if name.startswith("QUANTCONCLAVE_"):
        return os.environ.get(_LEGACY_ENV_PREFIX + name[len("QUANTCONCLAVE_"):])
    return None


def _getenv(name: str, default=None):
    """``os.getenv`` with a one-release ``CAPITALRADAR_*`` fallback."""
    val = _legacy_env(name)
    return val if val is not None and val != "" else default


def _apply_env_overrides(config: dict) -> dict:
    """Apply QUANTCONCLAVE_* env vars to the config dict in-place.

    ``CAPITALRADAR_*`` variables remain readable as a one-release
    compatibility fallback; the new prefix always wins.
    """
    for env_var, key in _ENV_OVERRIDES.items():
        raw = os.environ.get(env_var)
        if raw is None or raw == "":
            raw = _legacy_env(env_var)
        if raw is None or raw == "":
            continue
...
DEFAULT_CONFIG = _apply_env_overrides({
    ...
    "results_dir": _getenv("QUANTCONCLAVE_RESULTS_DIR", os.path.join(_QUANTCONCLAVE_HOME, "logs")),
    "data_cache_dir": _getenv("QUANTCONCLAVE_CACHE_DIR", os.path.join(_QUANTCONCLAVE_HOME, "cache")),
    ...
    "wecom_webhook_url": _getenv("QUANTCONCLAVE_WECOM_WEBHOOK_URL", ""),
    ...
    "wecom_bot_id": _getenv("QUANTCONCLAVE_WECOM_BOT_ID", ""),
    "wecom_bot_secret": _getenv("QUANTCONCLAVE_WECOM_BOT_SECRET", ""),
