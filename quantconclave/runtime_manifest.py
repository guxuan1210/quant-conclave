...
Values are overridable via ``QUANTCONCLAVE_WEB_MAIN_PORT`` /
``QUANTCONCLAVE_WEB_CHART_PORT`` (also settable in ``.env`` — dotenv loads
...
def _env(name: str) -> str | None:
    ...
    if name.startswith("QUANTCONCLAVE_"):
        return os.environ.get("CAPITALRADAR_" + name[len("QUANTCONCLAVE_"):])
    return None
...
def _services() -> dict[str, Service]:
    return {
        "main": Service(
            key="main",
            name="QuantConclave Dashboard",
            port=_port_from_env("QUANTCONCLAVE_WEB_MAIN_PORT", _MAIN_PORT_DEFAULT),
            description="主面板：深度分析、选股、预测、策略、回测、顾问、历史复盘、校准",
        ),
        "chart": Service(
            key="chart",
            name="QuantConclave Chart",
            port=_port_from_env("QUANTCONCLAVE_WEB_CHART_PORT", _CHART_PORT_DEFAULT),
