...
class ModelCapabilities:
    ...
    # Local Ollama reasoning models (e.g. qwen3.8) default thinking ON, which
    # makes even a tiny request take tens of seconds (22s+ measured on a tiny
    # prompt). Trading analysis is latency-sensitive and the prompts already
    # dictate output format, so the reasoning pass adds cost without value.
    # When set, OpenAIClient injects
    # ``chat_template_kwargs.enable_thinking=False`` for ollama-family
    # providers so every call site gets fast generation automatically.
...
# Local Qwen3.x reasoning models served via Ollama (qwen3, qwen3.5, qwen3.8,
# ...). Same permissive structured-output profile as _DEFAULT, but thinking is
# disabled by default (see the disable_thinking docstring) so single-stock
# analysis stays fast and deterministic across the whole family.
_QWEN3_LOCAL = ModelCapabilities(
    supports_tool_choice=True,
    supports_json_mode=True,
    supports_json_schema=True,
    preferred_structured_method="function_calling",
...
_BY_PATTERN: list[tuple[re.Pattern[str], ModelCapabilities]] = [
    ...
    # The whole Qwen3.x family (qwen3, qwen3.5:27b, qwen3.8:27b, ...) are
    # hybrid thinking models on Ollama — default them all to thinking OFF so
    # calls stay fast and deterministic.
    (re.compile(r"^qwen3(\.\d+)?"), _QWEN3_LOCAL),
]
