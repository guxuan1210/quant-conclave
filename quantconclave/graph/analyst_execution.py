...
# Single source of truth is quantconclave.catalog.ANALYST_ROLES; the specs below
# are a derived view so this module's existing API stays unchanged.
ANALYST_NODE_SPECS: Dict[str, AnalystNodeSpec] = {
    role.key: AnalystNodeSpec(
        key=role.key,
        agent_node=role.node,
        clear_node=role.clear_node,
        tool_node=role.tool_node,
        report_key=role.report_key,
    )
