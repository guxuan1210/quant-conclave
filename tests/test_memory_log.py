"""Tests for CapitalRadarMemoryLog — storage, deferred reflection, PM injection, legacy removal."""
...
from capitalradar.agents.schemas import PortfolioDecision, PortfolioRating
from capitalradar.graph.reflection import Reflector
...
_SEP = CapitalRadarMemoryLog._SEPARATOR
...
def make_log(tmp_path, filename="trading_memory.md"):
    ...
    return CapitalRadarMemoryLog(config)
...
class TestCapitalRadarMemoryLogCore:
    ...
    def test_no_log_path_is_noop(self):
        log = CapitalRadarMemoryLog(config=None)
        log.store_decision("NVDA", "2026-01-10", DECISION_BUY)
        assert log.load_entries() == []
        assert log.get_past_context("NVDA") == ""
...
class TestDeferredReflection:
    ...
    def test_fetch_returns_valid_ticker(self):
        ...
        with patch("yfinance.Ticker") as mock_ticker_cls:
            ...
            raw, alpha, days = CapitalRadarGraph._fetch_returns(mock_graph, "NVDA", "2026-01-05")
    ...
    def test_fetch_returns_too_recent(self):
        ...
        with patch("yfinance.Ticker") as mock_ticker_cls:
            ...
            raw, alpha, days = CapitalRadarGraph._fetch_returns(mock_graph, "NVDA", "2026-04-19")
    ...
    def test_fetch_returns_delisted(self):
        ...
        with patch("yfinance.Ticker") as mock_ticker_cls:
            ...
            mock_ticker_cls.return_value = m
            raw, alpha, days = CapitalRadarGraph._fetch_returns(mock_graph, "XXXXXFAKE", "2026-01-10")
        assert raw is None and alpha is None and days is None
    ...
    # CapitalRadarGraph._resolve_benchmark — picks index for alpha calc
    ...
    def test_resolve_benchmark_explicit_override(self):
        ...
        assert CapitalRadarGraph._resolve_benchmark(mock_graph, "7203.T") == "QQQ"
        assert CapitalRadarGraph._resolve_benchmark(mock_graph, "NVDA") == "QQQ"
    ...
    def test_resolve_benchmark_suffix_map(self):
        ...
        assert CapitalRadarGraph._resolve_benchmark(mock_graph, "7203.T") == "^N225"
        assert CapitalRadarGraph._resolve_benchmark(mock_graph, "0700.HK") == "^HSI"
        assert CapitalRadarGraph._resolve_benchmark(mock_graph, "RELIANCE.NS") == "^NSEI"
        assert CapitalRadarGraph._resolve_benchmark(mock_graph, "AZN.L") == "^FTSE"
    ...
    def test_resolve_benchmark_us_ticker_defaults_to_spy(self):
        ...
        assert CapitalRadarGraph._resolve_benchmark(mock_graph, "NVDA") == "SPY"
        assert CapitalRadarGraph._resolve_benchmark(mock_graph, "AAPL") == "SPY"
    ...
    def test_resolve_benchmark_unknown_suffix_falls_back(self):
        ...
        assert CapitalRadarGraph._resolve_benchmark(mock_graph, "FAKE.XX") == "SPY"
        assert CapitalRadarGraph._resolve_benchmark(mock_graph, "BRK.B") == "SPY"
    ...
    # CapitalRadarGraph._resolve_pending_entries
    ...
    def test_resolve_skips_other_tickers(self, tmp_path):
        ...
        CapitalRadarGraph._resolve_pending_entries(mock_graph, "NVDA")
...
class TestLegacyRemoval:
    ...
    def test_financial_situation_memory_removed(self):
        ...
        import capitalradar.agents.utils.memory as m
    ...
    def test_reflect_and_remember_removed(self):
        """CapitalRadarGraph must not expose reflect_and_remember."""
        assert not hasattr(CapitalRadarGraph, "reflect_and_remember")
    ...
    def test_full_pipeline_no_regression(self, tmp_path):
        ...
        mock_graph.memory_log = CapitalRadarMemoryLog({"memory_log_path": str(tmp_path / "mem.md")})
        ...
        mock_graph._run_graph = functools.partial(
            CapitalRadarGraph._run_graph, mock_graph
        )
        CapitalRadarGraph.propagate(mock_graph, "NVDA", "2026-01-10")
