"""Tests for QuantConclaveMemoryLog — storage, deferred reflection, PM injection, legacy removal."""
...
from quantconclave.agents.schemas import PortfolioDecision, PortfolioRating
from quantconclave.graph.reflection import Reflector
...
_SEP = QuantConclaveMemoryLog._SEPARATOR
...
def make_log(tmp_path, filename="trading_memory.md"):
    ...
    return QuantConclaveMemoryLog(config)
...
class TestQuantConclaveMemoryLogCore:
    ...
    def test_no_log_path_is_noop(self):
        log = QuantConclaveMemoryLog(config=None)
        log.store_decision("NVDA", "2026-01-10", DECISION_BUY)
        assert log.load_entries() == []
        assert log.get_past_context("NVDA") == ""
...
class TestDeferredReflection:
    ...
    def test_fetch_returns_valid_ticker(self):
        ...
        with patch("yfinance.Ticker") as mock_ticker_cls:
            ...
            raw, alpha, days = QuantConclaveGraph._fetch_returns(mock_graph, "NVDA", "2026-01-05")
    ...
    def test_fetch_returns_too_recent(self):
        ...
        with patch("yfinance.Ticker") as mock_ticker_cls:
            ...
            raw, alpha, days = QuantConclaveGraph._fetch_returns(mock_graph, "NVDA", "2026-04-19")
    ...
    def test_fetch_returns_delisted(self):
        ...
        with patch("yfinance.Ticker") as mock_ticker_cls:
            ...
            mock_ticker_cls.return_value = m
            raw, alpha, days = QuantConclaveGraph._fetch_returns(mock_graph, "XXXXXFAKE", "2026-01-10")
        assert raw is None and alpha is None and days is None
    ...
    # QuantConclaveGraph._resolve_benchmark — picks index for alpha calc
    ...
    def test_resolve_benchmark_explicit_override(self):
        ...
        assert QuantConclaveGraph._resolve_benchmark(mock_graph, "7203.T") == "QQQ"
        assert QuantConclaveGraph._resolve_benchmark(mock_graph, "NVDA") == "QQQ"
    ...
    def test_resolve_benchmark_suffix_map(self):
        ...
        assert QuantConclaveGraph._resolve_benchmark(mock_graph, "7203.T") == "^N225"
        assert QuantConclaveGraph._resolve_benchmark(mock_graph, "0700.HK") == "^HSI"
        assert QuantConclaveGraph._resolve_benchmark(mock_graph, "RELIANCE.NS") == "^NSEI"
        assert QuantConclaveGraph._resolve_benchmark(mock_graph, "AZN.L") == "^FTSE"
    ...
    def test_resolve_benchmark_us_ticker_defaults_to_spy(self):
        ...
        assert QuantConclaveGraph._resolve_benchmark(mock_graph, "NVDA") == "SPY"
        assert QuantConclaveGraph._resolve_benchmark(mock_graph, "AAPL") == "SPY"
    ...
    def test_resolve_benchmark_unknown_suffix_falls_back(self):
        ...
        assert QuantConclaveGraph._resolve_benchmark(mock_graph, "FAKE.XX") == "SPY"
        assert QuantConclaveGraph._resolve_benchmark(mock_graph, "BRK.B") == "SPY"
    ...
    # QuantConclaveGraph._resolve_pending_entries
    ...
    def test_resolve_skips_other_tickers(self, tmp_path):
        ...
        QuantConclaveGraph._resolve_pending_entries(mock_graph, "NVDA")
...
class TestLegacyRemoval:
    ...
    def test_financial_situation_memory_removed(self):
        ...
        import quantconclave.agents.utils.memory as m
    ...
    def test_reflect_and_remember_removed(self):
        """QuantConclaveGraph must not expose reflect_and_remember."""
        assert not hasattr(QuantConclaveGraph, "reflect_and_remember")
    ...
    def test_full_pipeline_no_regression(self, tmp_path):
        ...
        mock_graph.memory_log = QuantConclaveMemoryLog({"memory_log_path": str(tmp_path / "mem.md")})
        ...
        mock_graph._run_graph = functools.partial(
            QuantConclaveGraph._run_graph, mock_graph
        )
        QuantConclaveGraph.propagate(mock_graph, "NVDA", "2026-01-10")
