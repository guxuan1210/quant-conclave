...
def test_qwen3_8_injects_disable_thinking(monkeypatch):
    """ollama + qwen3.8 gets chat_template_kwargs.enable_thinking=False automatically."""
    monkeypatch.delenv("OLLAMA_BASE_URL", raising=False)
    mod = _reload_client()
    client = mod.OpenAIClient(model="qwen3.8:27b", provider="ollama")
    llm = client.get_llm()
    assert llm.extra_body["chat_template_kwargs"]["enable_thinking"] is False


def test_qwen3_5_injects_disable_thinking(monkeypatch):
    """The whole qwen3.x family on ollama gets thinking off, not just qwen3.8."""
    monkeypatch.delenv("OLLAMA_BASE_URL", raising=False)
    mod = _reload_client()
    client = mod.OpenAIClient(model="qwen3.5:27b", provider="ollama")
    llm = client.get_llm()
    assert llm.extra_body["chat_template_kwargs"]["enable_thinking"] is False


def test_non_qwen_ollama_model_no_thinking_injection(monkeypatch):
    """Non-qwen3 models on ollama must NOT get the injection."""
    monkeypatch.delenv("OLLAMA_BASE_URL", raising=False)
    mod = _reload_client()
    client = mod.OpenAIClient(model="gemma4:12b", provider="ollama")
    llm = client.get_llm()
    assert llm.extra_body is None or llm.extra_body.get("chat_template_kwargs") is None


def test_qwen3_8_injection_all_ollama_family(monkeypatch):
    """The injection applies to every ollama-family provider key."""
    monkeypatch.delenv("OLLAMA_BASE_URL", raising=False)
    mod = _reload_client()
    for prov in ("ollama", "ollama2", "ollama3", "ollama4"):
        client = mod.OpenAIClient(model="qwen3.8:27b", provider=prov)
        llm = client.get_llm()
        assert llm.extra_body["chat_template_kwargs"]["enable_thinking"] is False, prov


def test_caller_extra_body_is_merged_not_clobbered(monkeypatch):
    """A caller-supplied extra_body coexists with the injected thinking-off."""
    monkeypatch.delenv("OLLAMA_BASE_URL", raising=False)
    mod = _reload_client()
    client = mod.OpenAIClient(
    ...
    llm = client.get_llm()
    assert llm.extra_body["some_other_flag"] is True
    assert llm.extra_body["chat_template_kwargs"]["enable_thinking"] is False
