...
@pytest.mark.unit
class TestQwen3DisableThinking:
    """Local Qwen3.x Ollama models are hybrid thinking models — default thinking OFF."""

    def test_qwen3_family_disable_thinking(self):
        for model in (
            "qwen3", "qwen3:32b",
            "qwen3.5:27b", "qwen3.6:latest", "qwen3.7:35b",
            "qwen3.8:27b", "qwen3.8:latest", "qwen3.9:latest",
        ):
            caps = get_capabilities(model)
            assert caps.disable_thinking is True, model
            # still a permissive structured-output profile
            assert caps.preferred_structured_method == "function_calling", model
            assert caps.supports_tool_choice is True, model

    def test_unrelated_models_keep_default_thinking(self):
        for model in ("deepseek-chat", "gpt-4.1", "gemma4:12b", "llama3.1", "MiniMax-M2.7"):
            assert get_capabilities(model).disable_thinking is False
