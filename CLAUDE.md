...
**Pipeline via LangGraph**: The core is a 5-stage `StateGraph` at `quantconclave/graph/trading_graph.py` (`QuantConclaveGraph`). Stages:
...
5. **Output** — Full state written to disk as JSON + markdown under `~/.quantconclave/logs/<TICKER>/QuantConclaveStrategy_logs/`.

All stages share a single `AgentState` TypedDict (`quantconclave/agents/utils/agent_states.py`).

### Graph construction (`quantconclave/graph/`)

- **`trading_graph.py`** (`QuantConclaveGraph`) — Orchestrator: creates two LLM instances (deep-thinking for managers, quick-thinking for analysts/debaters), resolves pending memory-log entries before each run, handles `_fetch_returns` for outcome resolution, and coordinates graph invocation (stream for debug/CLI, invoke for web).
...
### Agent nodes (`quantconclave/agents/`)
...
**Memory (`agents/utils/memory.py`)**: `QuantConclaveMemoryLog` — append-only markdown decision log. Stores decisions immediately with `[pending]` tags; resolves outcomes later via `batch_update_with_outcomes()` using atomic temp-file writes. Past same-ticker decisions (up to 5) and cross-ticker lessons (up to 3) are injected into the Portfolio Manager prompt as `past_context`. Supports optional rotation via `memory_log_max_entries`.

### LLM clients (`quantconclave/llm_clients/`)
...
### Data vendors (`quantconclave/dataflows/`)
...
### Configuration (`quantconclave/default_config.py`)

`DEFAULT_CONFIG` is the single source of truth — a dict with env-var overrides via `QUANTCONCLAVE_*` variables. `_ENV_OVERRIDES` maps env var names to config keys; `_apply_env_overrides` coerces types based on existing default values. The `.env` file is loaded at `quantconclave/__init__.py` import time via `dotenv(find_dotenv(usecwd=True))` with `override=False`.
...
### Sector scan (`quantconclave/sector_scan/`)
...
- The `.env` file is loaded at `quantconclave/__init__.py` import time via `dotenv` with `usecwd=True`. `load_dotenv` uses `override=False` — existing env vars take precedence.
- All API key env var names are centralized in `quantconclave/llm_clients/api_key_env.py`.
...
- `_ENV_OVERRIDES` in `default_config.py` maps `QUANTCONCLAVE_*` env vars to config keys. Add a row there to expose a new key for env-based override.
